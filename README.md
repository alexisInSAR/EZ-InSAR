...
Please note that the processing with the Stripmap data has not been fully tested: only the preparation of data has been tested. However, the InSAR processors are compatible with them. For the Spotlight data, EZ-InSAR can manage the data similar to StripMap but the processing with ISCE should be modified. 
