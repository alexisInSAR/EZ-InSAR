...
"""
Module to manage COSMO-SkyMed Second-Generation images for EZ-InSAR
...
Changelog:
        * 3.1.0: Initial version, Feb. 2024
...
def initiateSLC(job,
        ...
        satellite: Optional[str] = 'CSKSG', 
        ):
        """Initialise the COSMO-SkyMed Second Generation SLC list 

        The function initialise the COSMO-SkyMed Second Generation SLC for an ``EIjob``. 
        ...
        usermessage.openingmsg(__name__,initiateSLC.__name__,__file__,__copyright__,'Create the SLC list for EZ-InSAR for COSMO-SkyMed Second Generation',job.log,verbose)

        if not job.satellite == None:
                if job.satellite == 'CSKSG': 
                        if mode == 'onfile':
                                usermessage.ezprint('Mode: base on files - the COSMO-SkyMed Second Generation SLC list will be created based on available files:',job.log,verbose)
                                job = createCSKSGlistonfile(job,verbose)
        else:
...
################################################################################
################################################################################
def createCSKSGlistonfile(job,verbose):
        """Create the COSMO-SkyMed Second Generation SLC list with the **onfile** mode

        The function initialise the COSMO-SkyMed Second Generation  SLC for an ``EIjob``, with the **onfile** mode.  
        ...
        listfile = glob.glob(job.pathSLC+os.sep+'CSG*')
        try:
        ...
        for filei in listfile:
                ...
                CSKannoresults = detectCSKSGannotation(filei,pol)
        ...
        if listSLCpd.empty == False:
                ...
                raise ValueError(usermessage.errormsg(__name__,createCSKSGlistonfile.__name__,__file__,__copyright__,'The list is empty.',job.log))

        return job 

def detectCSKSGannotation(file,pol):
        """Detect and read the annonation files from COSMO-SkyMed Second Generation file
        ...
        if not path_file: 
                raise ValueError(usermessage.errormsg(__name__,detectCSKSGannotation.__name__,__file__,__copyright__,'Impossible to detect the .h5 file in the directory %s' % (file),None))

        data_xmli1, data_orbitsi1  = read_CSKSGannotation(path_file[0]) 
      
        CSKSGannoresults = dict()
        CSKSGannoresults['data_xmli1'] = data_xmli1
        CSKSGannoresults['data_orbitsi1'] = data_orbitsi1

        return CSKSGannoresults

def read_CSKSGannotation(path_h5): 
        """Read the annonation files from the COSMO-SkyMed Second Generation file

        The function reads the annonation files from COSMO-SkyMed Second Generation file
        ...
        with h5py.File(path_h5, "r") as fslc:
                ...
                data_xml['polarisation'] = [fslc['/'].attrs['Polarization'].decode('utf-8').upper()] # Not sure for dual pol
                ...
                Zero_Doppler_Azimuth_First_Time = float(fslc['S01/IMG'].attrs['Zero Doppler Azimuth First Time'])
                Zero_Doppler_Azimuth_Last_Time = float(fslc['S01/IMG'].attrs['Zero Doppler Azimuth Last Time'])
                ...
                tmp = fslc['//S01/IMG'].shape
                nb_line = tmp[0]
                nb_pixel = tmp[1]

                tmp1 = fslc['/S01/IMG'].attrs['Bottom Left Geodetic Coordinates']
                tmp2 = fslc['/S01/IMG'].attrs['Bottom Right Geodetic Coordinates']
                tmp3 = fslc['/S01/IMG'].attrs['Top Right Geodetic Coordinates']
                tmp4 = fslc['/S01/IMG'].attrs['Top Left Geodetic Coordinates']
                ...
                data_xml['incidenceAngle'] = [float(fslc['S01/IMG'].attrs['Near Incidence Angle']),
                                        float(fslc['S01/IMG'].attrs['Far Incidence Angle']),
                                        float(fslc['S01/IMG'].attrs['Far Incidence Angle']),
                                        float(fslc['S01/IMG'].attrs['Near Incidence Angle']),
                                        float(fslc['S01/IMG'].attrs['Near Incidence Angle']),
                                        ]
