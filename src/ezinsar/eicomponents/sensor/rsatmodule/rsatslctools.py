...
"""
Module to manage RADARSAT-1 images for EZ-InSAR

The module allows to manage RADARSAT-1 images for an `EIjob`
...
def initiateSLC(job,
        ...
        satellite: Optional[str] = 'RSAT', 
        ):
        """Initialise the RSAT1 SLC list 

        The function initialise the RSAT1 SLC for an ``EIjob``. 
        ...
        if not server in ['ASF']:
                raise TypeError(usermessage.typeerrormsg(
                        __name__,initiateSLC.__name__,__file__,__copyright__,
                        'mode','"ASF"',job.log))
        
        usermessage.openingmsg(__name__,initiateSLC.__name__,__file__,__copyright__,'Create the SLC list for EZ-InSAR for RadarSAT-1',job.log,verbose)

        if not job.satellite == None:
                if job.satellite == 'RSAT': 
                        if mode == 'onfile':
                                usermessage.ezprint('Mode: base on files - the RSAT SLC list will be created based on available files:',job.log,verbose)
                                job = createRSATlistonfile(job,verbose)
                        elif mode == 'online':
                                if not beam == None:
                                        usermessage.ezprint('Mode: online - the RADARSAT SLC list will be created based on the job information:',job.log,verbose)
                                        ...
                                        usermessage.ezprint('Beam for %s' % (beam),job.log,verbose)
                                        ...
                                        job = createRSATlistonline(job,verbose,server,satellite,beam,polarisation=job.polarisation,frame=frame)
                                else:
                                        raise ValueError(usermessage.errormsg(__name__,initiateSLC.__name__,__file__,__copyright__,'The beam is required.',job.log))
...
################################################################################
################################################################################
def createRSATlistonfile(job,verbose,server,satellite,beam,polarisation=['VV'],frame=None):
        """Create the RSAT SLC list with the **online** mode

        The function initialise the RSAT SLC for an ``EIjob``. 
        ...
        if listSLCpd.empty == False:
                ...
                raise ValueError(usermessage.errormsg(__name__,createRSATlistonline.__name__,__file__,__copyright__,'The list is empty.',job.log))
